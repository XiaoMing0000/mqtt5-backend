...
export class MqttManager {
  ...
  public async publishHandle(pubData: IPublishData, emitAsync: (client: TClient, event: string, ...args: any[]) => Promise<boolean>) {
    try {
      ...
      if (
        this.connData.properties.maximumPacketSize &&
        (pubData.header.remainingLength ?? 0) > (this.connData.properties.maximumPacketSize ?? 1 << 20)
      ) {
        throw new DisconnectException(
   ...
   */
  public async handleUnsubscribeAck(
    unsubscribeData: IUnsubscribeData,
    protocolVersion: ProtocolVersion,
    reasonCodes: number[] = [UnsubscribeAckReasonCode.Success],
  ) {
